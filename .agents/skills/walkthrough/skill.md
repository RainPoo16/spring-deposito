...
**Use the #runSubagent tool to delegate exploration to subagents.** This keeps the main context clean for HTML generation and parallelizes the research phase.
...
#### 2b. Launch parallel Explorer subagents

Use `#runSubagent`: `Explorer` to launch multiple agents **in a single message** (parallel). Each agent should investigate one area and report back the **purpose and connections** of each piece — not code dumps.
...
**CRITICAL**: You MUST follow the complete template in [references/html-patterns.md](references/html-patterns.md). Do NOT create a simplified version.

**Reference styling implementation**: See [references/example.html](references/example.html) for a complete example of a generated styling HTML file.

**File location**: Write to the project `/walkthrough` folder as `walkthrough-{topic}.html` (e.g., `walkthrough-canvas-drawing.html`). Use kebab-case for the topic slug.
...
**CRITICAL: Click Handler Setup**

For click handlers to work, you MUST:

1. **Set `securityLevel: 'loose'`** in `mermaid.initialize()`:
   ```js
   mermaid.initialize({
     startOnLoad: true,
...
2. **Define `window.nodeClickHandler` BEFORE `mermaid.initialize()`**:
   ```js
   // Define click handler first
   window.nodeClickHandler = function(nodeId) {
     // handler logic
   };
...
Without `securityLevel: 'loose'`, Mermaid ignores all click bindings for security reasons.
...
- [ ] Make sure the html renders correctly in a browser
...
- [references/example.html](references/example.html) — Example generated walkthrough HTML file
